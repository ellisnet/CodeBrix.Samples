...
public static class PaletteHelper
{
	...
	public static void Save (this Palette palette, string path, IPaletteSaver saver)
	{
		saver.Save (palette.Colors, path);
	}
	...
	public static void Load (this Palette palette, PaletteFormatManager paletteFormats, string path)
	{
		List<Color> loadedColors = LoadColorsFromFile (paletteFormats, path);
		palette.Load (loadedColors);
	}

	private static List<Color> LoadColorsFromFile (PaletteFormatManager paletteFormats, string path)
	{
		var loader = paletteFormats.GetFormatByFilename (System.IO.Path.GetFileName (path))?.Loader;
		...
		foreach (var format in paletteFormats.Formats.Where (f => !f.IsWriteOnly ())) {
			try {
				var loaded_colors = format.Loader.Load (path);
				if (loaded_colors != null)
					return loaded_colors;
			} catch (Exception e) {
				// Record errors in case none of the formats work.
				errors.AppendLine ($"Failed to load palette as {format.FilterName}:");
