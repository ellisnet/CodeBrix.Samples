...
/// double up with the app's own publish-time overwrite prompt. This dialog lets us clear the
...
internal static class Win32SaveFileDialog
{
    ...
    public static string PickSavePath(IntPtr ownerHwnd, string suggestedFileName, string title)
    {
        ...
        try
        {
            ...
            var filters = new[]
            {
                new COMDLG_FILTERSPEC { pszName = "PDF document", pszSpec = "*.pdf" },
            ...
            TrySetDefaultFolder(dialog, Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments));
    ...
    private static void TrySetDefaultFolder(IFileDialog dialog, string folderPath)
    {
        ...
        {
            //A missing/inaccessible Documents folder just means the dialog opens wherever it likes.
        }
    }
