...
internal static class BookFonts
{
    ...
    public static void EnsureRegistered()
    {
        lock (_locker)
        {
            ...
            var monoFaces = new[] { "SourceCodePro-Regular" };
            var monoResolver = new EmbeddedFontResolver(
                fontFamilyName: MonoFamily,
                fontFaceResources:
                [
                    new EmbeddedResourceFontFace(FaceName: "SourceCodePro-Regular", EmbeddedResourceName: $"{ResourcePrefix}SourceCodePro-Regular.ttf")
                ],
                fontEmbeddedResourceAssembly: assembly);

            var emojiFaces = new[] { "NotoEmoji-Regular" };
            var emojiResolver = new EmbeddedFontResolver(
                fontFamilyName: EmojiFamily,
                fontFaceResources:
                [
                    new EmbeddedResourceFontFace(FaceName: "NotoEmoji-Regular", EmbeddedResourceName: $"{ResourcePrefix}NotoEmoji-Regular.ttf")
                ],
                fontEmbeddedResourceAssembly: assembly);
            ...
            foreach (var face in monoFaces)
            {
                MetaFontResolver.Instance.RegisterFontResolver(face, monoResolver);
            }
            foreach (var face in emojiFaces)
            {
                MetaFontResolver.Instance.RegisterFontResolver(face, emojiResolver);
            }
