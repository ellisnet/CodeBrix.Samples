...
/// <summary>
/// Exercises <see cref="VulkanSceneRenderer"/> against a real Vulkan implementation (a GPU
/// driver or CPU lavapipe). Skips when no Vulkan stack is available. Mirrors
/// <see cref="GlModelSceneRendererTests"/> so the two backends prove the same behaviors.
/// </summary>
...
public class VulkanSceneRendererTests
{
    ...
    public void renderer_draws_a_triangle_onto_the_background()
    {
        ...
        RequireVulkan();
        const int size = 64;
        using var renderer = new VulkanSceneRenderer();
    ...
    {
        //Arrange - the same rotated-camera depth-ordering regression as the GL renderer: a
        //bad MVP transpose collapses the depth axis only for non-axis-aligned cameras, so a
        //near red triangle must beat a far blue one at the center from a rotated view no
        //matter the draw order.
        RequireVulkan();
        const int size = 32;
    ...
    public void textured_material_shows_its_texture_color()
    {
        ...
        //proving the staging-buffer upload, mip chain, sampler, and descriptor binding work.
        RequireVulkan();
        const int size = 64;
