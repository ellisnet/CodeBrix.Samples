...
public class SettingsStoreTests : IDisposable
{
    ...
    public void Get_returns_default_when_not_set()
    {
        ...
        //Assert
        store.Get("KenneyAssetBrowser.Test.Missing", 42).Should().Be(42);
        store.Get<string>("KenneyAssetBrowser.Test.Missing").Should().BeNull();
        store.HasValue("KenneyAssetBrowser.Test.Missing").Should().BeFalse();
    }
    ...
    public void Set_and_Get_round_trip_common_types()
    {
        ...
        //Act
        store.Set("KenneyAssetBrowser.Test.String", "hello");
        store.Set("KenneyAssetBrowser.Test.Int", 7);
        store.Set("KenneyAssetBrowser.Test.Bool", true);
        store.Set("KenneyAssetBrowser.Test.Enum", DayOfWeek.Friday);

        //Assert
        store.Get<string>("KenneyAssetBrowser.Test.String").Should().Be("hello");
        store.Get<int>("KenneyAssetBrowser.Test.Int").Should().Be(7);
        store.Get<bool>("KenneyAssetBrowser.Test.Bool").Should().BeTrue();
        store.Get<DayOfWeek>("KenneyAssetBrowser.Test.Enum").Should().Be(DayOfWeek.Friday);
    }
    ...
    public void Values_persist_across_reopen()
    {
        //Arrange
        using (var store = CreateStore())
            store.Set("KenneyAssetBrowser.Test.Persisted", "survives");
        ...
        reopened.Get<string>("KenneyAssetBrowser.Test.Persisted").Should().Be("survives");
    }
    ...
    public void Set_null_removes_the_key()
    {
        //Arrange
        using var store = CreateStore();
        store.Set("KenneyAssetBrowser.Test.Removed", "value");

        //Act
        store.Set("KenneyAssetBrowser.Test.Removed", null);

        //Assert
        store.HasValue("KenneyAssetBrowser.Test.Removed").Should().BeFalse();
    }
    ...
    public void Set_returns_true_only_when_the_value_changes()
    {
        ...
        //Assert
        store.Set("KenneyAssetBrowser.Test.Changed", "a").Should().BeTrue();
        store.Set("KenneyAssetBrowser.Test.Changed", "a").Should().BeFalse();
        store.Set("KenneyAssetBrowser.Test.Changed", "b").Should().BeTrue();
    }
    ...
    public void Set_raises_change_events()
    {
        ...
        store.AddSettingHandler("KenneyAssetBrowser.Test.Watched", (_, args) => keyed = args);

        //Act
        store.Set("KenneyAssetBrowser.Test.Watched", "new");

        //Assert
        broadcast.Should().NotBeNull();
        broadcast.Key.Should().Be("KenneyAssetBrowser.Test.Watched");
    ...
    public void Autobackup_is_a_complete_usable_database()
    {
        ...
        using (var store = CreateStore())
            store.Set("KenneyAssetBrowser.Test.InBackup", "captured");
        using (CreateStore()) { }
        ...
        //Assert
        restored.Get<string>("KenneyAssetBrowser.Test.InBackup").Should().Be("captured");
    }
    ...
    public void Corrupt_file_is_quarantined_and_restored_from_newest_backup()
    {
        ...
        using (var store = CreateStore())
            store.Set("KenneyAssetBrowser.Test.Value", "from-backup");
        using (CreateStore()) { }
        ...
        store2.Get<string>("KenneyAssetBrowser.Test.Value").Should().Be("from-backup");
    ...
    public void Mismatched_type_read_returns_the_default()
    {
        //Arrange
        using var store = CreateStore();
        store.Set("KenneyAssetBrowser.Test.Typed", "not a number");

        //Assert
        store.Get("KenneyAssetBrowser.Test.Typed", 5).Should().Be(5);
    }
    ...
    public void Export_writes_a_complete_self_contained_copy()
    {
        //Arrange
        using var store = CreateStore();
        store.Set("KenneyAssetBrowser.Test.Exported", "travels");
        ...
        reopened.Get<string>("KenneyAssetBrowser.Test.Exported").Should().Be("travels");
    }
    ...
    public void Import_stages_a_validated_incoming_file()
    {
        //Arrange
        using var store = CreateStore();
        var sourcePath = CreateExternalSettingsFile("KenneyAssetBrowser.Test.Imported", "incoming");
    ...
    public void Incoming_file_is_adopted_on_startup()
    {
        ...
        var sourcePath = CreateExternalSettingsFile("KenneyAssetBrowser.Test.Key", "new");
        using (var store = CreateStore())
        {
            store.Set("KenneyAssetBrowser.Test.Key", "old");
            store.StageIncomingFile(sourcePath);
        }
        ...
        reopened.WasCreatedFresh.Should().BeFalse();
        reopened.Get<string>("KenneyAssetBrowser.Test.Key").Should().Be("new");
        File.Exists(Path.Combine(directory, AppSettingsStore.IncomingFileName)).Should().BeFalse();
    ...
    public void Adoption_without_an_existing_settings_file_uses_the_incoming_file_directly()
    {
        ...
        var sourcePath = CreateExternalSettingsFile("KenneyAssetBrowser.Test.Key", "fresh-import");
        Directory.CreateDirectory(directory);
        ...
        store.Get<string>("KenneyAssetBrowser.Test.Key").Should().Be("fresh-import");
    ...
    public void Facade_initializes_reads_writes_and_shuts_down()
    {
        ...
        try
        {
            ...
            SettingsService.Set("KenneyAssetBrowser.Test.Facade", "works");
            SettingsService.HasValue("KenneyAssetBrowser.Test.Facade").Should().BeTrue();
            SettingsService.Get<string>("KenneyAssetBrowser.Test.Facade").Should().Be("works");
            SettingsService.Get("KenneyAssetBrowser.Test.FacadeMissing", 3).Should().Be(3);
        }
        finally
        {
