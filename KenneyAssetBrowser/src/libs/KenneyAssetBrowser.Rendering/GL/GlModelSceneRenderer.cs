...
public sealed class GlModelSceneRenderer : IModelSceneRenderer
{
    ...
    /// <inheritdoc />
    /// <param name="frame">Per-primitive vertex data, aligned with the model's primitives.</param>
    public void SetFrameVertices(IReadOnlyList<ModelFramePrimitive>? frame)
    {
        lock (_pendingLock)
        {
            _pendingFrame = frame;
    ...
    private IReadOnlyList<ModelFramePrimitive>? _pendingFrame;

    //Overwrites the position/normal buffers with a queued animation frame (GL thread)
    private void ApplyPendingFrame(GL gl)
    {
        IReadOnlyList<ModelFramePrimitive>? frame;
        lock (_pendingLock)
        {
            frame = _pendingFrame;
            _pendingFrame = null;
        ...
        if (frame == null || frame.Count != _gpuPrimitives.Count) { return; }

        for (var i = 0; i < frame.Count; i++)
        {
            gl.BindBuffer(BufferTargetARB.ArrayBuffer, _gpuPrimitives[i].PositionBuffer);
            gl.BufferSubData<float>(BufferTargetARB.ArrayBuffer, 0, frame[i].Positions.AsSpan());
            gl.BindBuffer(BufferTargetARB.ArrayBuffer, _gpuPrimitives[i].NormalBuffer);
            gl.BufferSubData<float>(BufferTargetARB.ArrayBuffer, 0, frame[i].Normals.AsSpan());
        }
    ...
    public unsafe void Render(GL gl, uint width, uint height)
    {
        ...
        ApplyPendingFrame(gl);

        gl.Viewport(0, 0, width, height);
        var (r, g, b, a) = BackgroundColor;
        gl.ClearColor(r, g, b, a);
        gl.Enable(EnableCap.DepthTest);
        gl.Disable(EnableCap.CullFace); // Many low-poly game models rely on double-sided rendering
