...
- [BLUEPRINTS-AppStructureAndStartup.md](BLUEPRINTS-AppStructureAndStartup.md) - Everything that has to happen before the first page appears: head entry points, the ordering contract inside the App constructor, service registration, logging, fonts, the main window and the per-head opt-ins (13 recipes)
- [BLUEPRINTS-MVVM.md](BLUEPRINTS-MVVM.md) - The view model layer: bound properties and commands, the design-mode guard, long jobs with progress and cancellation, results marshalled back to the UI thread, dialogs raised from a view model, and orderly disposal (46 recipes)
- [BLUEPRINTS-PlatformServices.md](BLUEPRINTS-PlatformServices.md) - The bridge interfaces and settable delegates that let a view model reach what only a page or a head can do: dialogs, native pickers, the clipboard, canvas repaints, timers, an embedded browser and an audio transport (15 recipes)
- [BLUEPRINTS-ViewsAndControls.md](BLUEPRINTS-ViewsAndControls.md) - Markup and page code-behind: XAML namespaces and data contexts, theme brush keys, value converters, layouts that reflow, the controls you write yourself, and forwarding pointer, wheel and keyboard input (31 recipes)
...
- [BLUEPRINTS-DocumentsAndData.md](BLUEPRINTS-DocumentsAndData.md) - Work with content rather than pixels, kept in UI-free libraries behind service interfaces: archives and embedded resources, encryption, REST and HTTP calls, parsing messy markup, and producing, reading and saving documents (43 recipes)
