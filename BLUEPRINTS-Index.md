...
- [BLUEPRINTS-AppStructureAndStartup.md](BLUEPRINTS-AppStructureAndStartup.md) - Everything that has to happen before the first page appears: head entry points, the ordering contract inside the App constructor, service registration, logging, fonts, the main window and its size, and the per-head opt-ins (17 recipes)
- [BLUEPRINTS-MVVM.md](BLUEPRINTS-MVVM.md) - The view model layer: bound properties and commands, the design-mode guard, long jobs with progress and cancellation, results marshalled back to the UI thread, dialogs raised from a view model, and orderly disposal (55 recipes)
- [BLUEPRINTS-PlatformServices.md](BLUEPRINTS-PlatformServices.md) - The bridge interfaces and settable delegates that let a view model reach what only a page or a head can do: dialogs, native pickers, the clipboard, canvas repaints, timers, an embedded browser and an audio transport (22 recipes)
- [BLUEPRINTS-ViewsAndControls.md](BLUEPRINTS-ViewsAndControls.md) - Markup and page code-behind: XAML namespaces and data contexts, theme brush keys, value converters, layouts that reflow, the controls you write yourself, forwarding pointer, wheel and keyboard input, and the PasswordBox mask character (42 recipes)
- [BLUEPRINTS-ThemingAndStyling.md](BLUEPRINTS-ThemingAndStyling.md) - How an application decides what it looks like: a palette written as plain data, following or overriding the desktop appearance, repainting every scheme at run time, re-keying the stock control families, brushes that belong to an item, a visual language of borders pills and hairlines, the shipped symbols font, proving a capability before designing around it, and keeping a text box's own colors while it is hovered or focused (11 recipes)
...
- [BLUEPRINTS-MediaAndVision.md](BLUEPRINTS-MediaAndVision.md) - Playing video and audio inside a page, probing and converting media files, and driving camera capture and on-device vision without leaking device libraries into the view model (23 recipes)
- [BLUEPRINTS-DocumentsAndData.md](BLUEPRINTS-DocumentsAndData.md) - Work with content rather than pixels, kept in UI-free libraries behind service interfaces: archives and embedded resources, encryption, REST and HTTP calls, parsing messy markup, and producing, reading and saving documents (46 recipes)
- [BLUEPRINTS-SettingsAndPersistence.md](BLUEPRINTS-SettingsAndPersistence.md) - Keeping state between runs through the AppSettings add-in, from the application-named facade over the store to when that store must open relative to the rest of startup, and the case for no store at all when the thing you create can carry its own identity (7 recipes)
...
- [BLUEPRINTS-GameEngine.md](BLUEPRINTS-GameEngine.md) - Hosting the CodeBrix.Platform GameEngine loop inside an ordinary page and owning the engine lifecycle in a session class that can start, pause, resume and stop, and driving the engine's music system: the pinned device format, the beat grid a track is handed or brings with it, bar-locked transitions across a tempo change, layered stems, ducking and the pause overlay (8 recipes)
- [BLUEPRINTS-Testing.md](BLUEPRINTS-Testing.md) - Getting an application under test: test project setup, fixtures and test doubles, headless graphics and golden-image comparison, opt-in live tests, and a scripted run that drives the whole application on a real head (27 recipes)
- [BLUEPRINTS-ProjectLayoutAndPackaging.md](BLUEPRINTS-ProjectLayoutAndPackaging.md) - How a multi-head application is laid out on disk and how its packages are apportioned: the shared project, root namespaces, the src and tests layout, native payloads, separate solutions and a notices file (19 recipes)
