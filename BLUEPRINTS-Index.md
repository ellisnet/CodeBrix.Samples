...
- [BLUEPRINTS-AppStructureAndStartup.md](BLUEPRINTS-AppStructureAndStartup.md) - Everything that has to happen before the first page appears: head entry points, the ordering contract inside the App constructor, service registration - including a factory for what a view model must own rather than share - logging, fonts, the main window and its size, and the per-head opt-ins, down to the folders a framebuffer picker is handed (19 recipes)
- [BLUEPRINTS-MVVM.md](BLUEPRINTS-MVVM.md) - The view model layer: bound properties and commands, the design-mode guard, long jobs with progress and cancellation, results marshalled back to the UI thread, work started where nothing can await it, dialogs raised from a view model, and orderly disposal of what the view model actually owns (68 recipes)
- [BLUEPRINTS-PlatformServices.md](BLUEPRINTS-PlatformServices.md) - The bridge interfaces and settable delegates that let a view model reach what only a page or a head can do: dialogs, native pickers, the clipboard, canvas repaints, timers, an embedded browser, an audio transport and the signal that the page is on screen - and the same interfaces carrying a page's paint call back the other way (29 recipes)
- [BLUEPRINTS-ViewsAndControls.md](BLUEPRINTS-ViewsAndControls.md) - Markup and page code-behind: XAML namespaces and data contexts, theme brush keys, value converters, layouts that reflow, the controls you write yourself, forwarding pointer, wheel and keyboard input, how little wiring a page needs - one subscription dropped on unload, a getter passed instead of an object that has not arrived - and the PasswordBox mask character (55 recipes)
- [BLUEPRINTS-ThemingAndStyling.md](BLUEPRINTS-ThemingAndStyling.md) - How an application decides what it looks like: a palette written as plain data, following or overriding the desktop appearance while the library decides what that preference means, repainting every scheme at run time, re-keying the stock control families, brushes that belong to an item including the caption color it settles for its own ink, a visual language of borders pills and hairlines, the shipped symbols font, proving a capability before designing around it, and keeping a text box's own colors while it is hovered or focused (14 recipes)
- [BLUEPRINTS-GraphicsAndRendering.md](BLUEPRINTS-GraphicsAndRendering.md) - Producing pixels rather than arranging controls: 3D scenes behind headless interfaces, per-head backend selection, off-screen rendering and compositing, canvas painting, and decoding, rasterizing and showing images (48 recipes)
- [BLUEPRINTS-MediaAndVision.md](BLUEPRINTS-MediaAndVision.md) - Playing video and audio inside a page, probing and converting media files, and driving camera capture and on-device vision without leaking device libraries into the view model: switching devices off the UI thread while the service that owns the device serializes every control call, and handing the newest frame to a renderer through a one-call seam the camera library declares (25 recipes)
- [BLUEPRINTS-DocumentsAndData.md](BLUEPRINTS-DocumentsAndData.md) - Work with content rather than pixels, kept in UI-free libraries behind service interfaces: archives and embedded resources, encryption, REST and HTTP calls, parsing messy markup, and producing, reading and saving documents - including letting the format registry, rather than a page, say what a file dialog offers (55 recipes)
- [BLUEPRINTS-SettingsAndPersistence.md](BLUEPRINTS-SettingsAndPersistence.md) - Keeping state between runs through the AppSettings add-in, from the application-named facade over the store to when that store must open relative to the rest of startup, remembering a folder choice and a window size, flushing deferred writes at natural points, and the case for no store at all when the thing you create can carry its own identity (7 recipes)
- [BLUEPRINTS-TextEditing.md](BLUEPRINTS-TextEditing.md) - Text that no XAML text control is drawing: laying it out through the TextLayout add-in from a headless library, so shaping, measurement, caret and selection geometry and an outline path are available with no text control involved, and keeping the escape sequences a terminal understands inside the library that owns the terminal (2 recipes)
- [BLUEPRINTS-GameEngine.md](BLUEPRINTS-GameEngine.md) - Hosting the CodeBrix.Platform GameEngine loop inside an ordinary page - the canvas reaching the view model as a host interface, the session built by a registered factory and able to start, pause, resume and stop - and driving the engine's music system: the pinned device format, the beat grid a track is handed or brings with it, bar-locked transitions across a tempo change, layered stems, ducking and the pause overlay (8 recipes)
- [BLUEPRINTS-Testing.md](BLUEPRINTS-Testing.md) - Getting an application under test: test project setup, fixtures and test doubles, headless graphics and golden-image comparison, opt-in live tests, proving a registration extension registers what it promises, a scripted run kept in a library behind a narrow interface onto the page, and a diagnostic hook that drives a live application from an environment variable (32 recipes)
- [BLUEPRINTS-ProjectLayoutAndPackaging.md](BLUEPRINTS-ProjectLayoutAndPackaging.md) - How a multi-head application is laid out on disk and how its packages are apportioned: the shared project, root namespaces, the src and tests layout, native payloads, separate solutions, a notices file, the loose source files linked into each head with a symbol that picks its UI stack, and keeping a library's public surface to the one type its host drives (21 recipes)
...
- [View models, commands and threading](BLUEPRINTS-MVVM.md)
  ...
  - [Dispose only the service the view model built itself](BLUEPRINTS-MVVM.md#dispose-only-the-service-the-view-model-built-itself)
...
- [Views, XAML and custom controls](BLUEPRINTS-ViewsAndControls.md)
  ...
  - [Set the PasswordBox mask character back to WinUI's black circle](BLUEPRINTS-ViewsAndControls.md#set-the-passwordbox-mask-character-back-to-winuis-black-circle)
...
- [Theming and styling](BLUEPRINTS-ThemingAndStyling.md)
  ...
  - [Let a domain object carry the caption color for its own ink](BLUEPRINTS-ThemingAndStyling.md#let-a-domain-object-carry-the-caption-color-for-its-own-ink)
